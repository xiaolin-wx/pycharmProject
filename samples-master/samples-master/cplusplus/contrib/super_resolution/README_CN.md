...
3. 获取样例需要的测试图片。
     ...
     **cd $HOME/samples/cplusplus/contrib/super_resolution/data**

     **wget https://c7xcode.obs.cn-north-4.myhuaweicloud.com/models/super_resolution/test_image/butterfly_GT.bmp** 
...
3. 切换到super_resolution目录，创建目录用于存放编译文件，例如，本文中，创建的目录为 **build/intermediates/host**。

    **cd $HOME/samples/cplusplus/contrib/super_resolution/**
...
5. 执行make命令，生成的可执行文件main在 **super_resolution/out** 目录下。
...
2. <a name="step_2"></a>运行可执行文件。
    ...
    - 如果是开发环境与运行环境合一部署，执行以下命令，设置运行环境变量，并切换目录。
      ...
      **cd \$HOME/samples/cplusplus/contrib/super_resolution/out**

    - 如果是开发环境与运行环境分离部署，执行以下命令切换目录。
    
      **cd \$HOME/super_resolution/out**
...
运行完成后，会在运行环境的命令行中打印出推理结果,并在$HOME/super_resolution/out/output目录下生成推理后的图片。
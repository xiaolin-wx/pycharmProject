 ...
 **cd $HOME/samples/python/level2_simple_inference/2_object_detection/YOLOV3_coco_detection_picture_with_postprocess_op/model** 
    ...
    |  **模型名称**  |  **模型说明**  |  **模型下载路径**  |
    |---|---|---|
    |  yolov3| 基于Tensorflow-YOLOV3的目标检测模型。  |  请参考[https://gitee.com/ascend/modelzoo/tree/master/contrib/TensorFlow/Research/cv/yolov3/ATC_yolov3_tensorflow](https://gitee.com/ascend/modelzoo/tree/master/contrib/TensorFlow/Research/cv/yolov3/ATC_yolov3_tensorflow)目录中README.md下载原始模型章节下载模型。 |
    |  yolov3_output_op | 基于Caffe-YOLOV3的单算子模型。  |  请参考[https://gitee.com/ascend/modelzoo/tree/master/contrib/TensorFlow/Research/cv/yolov3_output_op](https://gitee.com/ascend/modelzoo/tree/master/contrib/TensorFlow/Research/cv/yolov3_output_op)目录中README.md下载原始模型章节下载模型和权重文件。 |
...
2. <a name="step_2"></a>运行可执行文件。
    ...
    - 如果是开发环境与运行环境合一部署，执行以下命令，设置运行环境变量，并切换目录。
      ...
      **cd $HOME/samples/python/level2_simple_inference/2_object_detection/YOLOV3_coco_detection_picture_with_postprocess_op/src**    
...
运行完成后，会在运行环境的命令行中打印出推理结果，并且在工程目录的outputs文件夹下保存推理结果的图片。

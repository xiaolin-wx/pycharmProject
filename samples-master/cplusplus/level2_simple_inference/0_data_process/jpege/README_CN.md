...
2. 获取样例需要的测试图片。
    ...
    **cd $HOME/samples/cplusplus/level2_simple_inference/0_data_process/jpege/data**
...
2. 切换到jpege目录，创建目录用于存放编译文件，例如，本文中，创建的目录为 **build/intermediates/host**。

    **cd $HOME/samples/cplusplus/level2_simple_inference/0_data_process/jpege**
...
4. 执行make命令，生成的可执行文件main在 **jpege/out** 目录下。
...
1. 执行以下命令,将开发环境的 **jpege** 目录上传到运行环境中，例如 **/home/HwHiAiUser**，并以HwHiAiUser（运行用户）登录运行环境（Host）。
...
2. <a name="step_2"></a>运行可执行文件。
    ...
    - 如果是开发环境与运行环境合一部署，执行以下命令，设置运行环境变量，并切换目录。
      ...
      **cd $HOME/samples/cplusplus/level2_simple_inference/0_data_process/jpege/out**
...
运行完成后，会在运行环境的命令行中打印出推理结果,并在$HOME/jpege/out/output目录下生成推理后的图片。
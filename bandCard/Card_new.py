...
class Card(object):
    ...
    # Canny轮廓检测
    def check_canny(self, img, threshold1, aperturesize, threshold2):
    ...
    # 图像透视变化
    def transform_img(self, img, points1, points2, size):
        # 变换矩阵 M
        M = cv.getAffineTransform(points1, points2)
        # 变换后的图像
        transform_img = cv.warpAffine(img, M, size)
        return transform_img

    # 图片透视
    def transform_bigimg(self, img, contours,a,b,c):
        res1_img = img.copy()
    ...
    def transform_bigimg_9(self, img, contours,a,b,c):
        res1_img = img.copy()

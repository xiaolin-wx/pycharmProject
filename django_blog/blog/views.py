...
class CommonResponse:
    ...
    @property#Python的@property使用方法详解   将类方法转换为类属性，可以用 . 直接获取属性值或者对属性进行赋值
    def all(self):
...
def register(request):
    if request.method == 'GET':
        form = x_forms.FormRegister()# 通过x_forms调用formregister调用方法 并且提示向相应参数
        return render(request, 'Register.html', context={'form': form})
    elif request.method == 'POST':
        form = x_forms.FormRegister(data=request.POST)#同上
        res = {'code': '', 'msg': ''}
        if form.is_valid():
...
def get_valid_code(request):
    ...
    valid_code = ''
    for i in range(5):
        low_char = chr(random.randint(97, 122))#随机获取数字
        num_char = random.randint(0, 9)#同上
        upper_char = chr(random.randint(65, 90))#同上
...
def C(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=1).all()
...
def Cjj(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=5).all()
    ...
    if page_num_int > int(end):
        # 页码出错转到报错界面
        return redirect(reverse('error'))
    return render(request, 'Cjj.html', locals())


# python专题
def python(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=4).all()
...
def Java(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=2).all()
...
def MySQL(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=3).all()
    ...
    if page_num_int > int(end):
        # 页码出错转到报错界面
        return redirect(reverse('error'))
    return render(request, 'MySQL.html', locals())


# wbe专题
def web(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=6).all()
...
def iOS(request):
    # 获取
    article_list = models.Article.objects.filter(category_id=7).all()
    ...
    if page_num_int > int(end):
        # 页码出错转到报错界面
        return redirect(reverse('error'))
    return render(request, 'iOS.html', locals())
...
def back(request):
    ...
    category_list = models.Category.objects.filter(blog=blog)
    tag_list = models.Tag.objects.filter(blog=blog)
    return render(request, 'user/back.html', locals())
